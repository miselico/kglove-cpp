...
public:
     ...
     */
    Co_occurenceComputer(const string & inputGraphFileName, const bool removeLiterals) {
        pair<shared_ptr<QuickGraph::LabeledGraph>, unordered_map<string, unsigned int> > graphAndNodeIndex = n3parser::buildRDFGraph(inputGraphFileName, removeLiterals);
        _weightedGraph = graphAndNodeIndex.first;
...
//	}
     ...
     *
     */
    void computeFrequenciesNoEdges(double bca_alpha, double bca_eps, FILE * glove_input_file_out, bool onlyEntities) {
     ...
     *
     */
    void computeFrequenciesIncludingEdges(double bca_alpha, double bca_eps, FILE * glove_input_file_out, bool onlyEntities) {
...
void parametrizedRun(Parameters& param) {
    ...
    for (std::vector<std::tuple<std::string, bool, bool>>::const_iterator graphs = param.graphs.begin(); graphs != param.graphs.end(); graphs++) {
        ...
        co_occurence_computer::Co_occurenceComputer c(graphInputPath.c_str(), removeLiterals);
        if (saveVocabulary) {
            ...
            for (auto includeEdges : param.includeEdges) {
                ...
                if (includeEdges) {
                    vocab_out_path += (start + "_with_edges_" + graphFilename_replaced_dots + ".txt");
                } else {
                    vocab_out_path += (start + "_no_edges_" + graphFilename_replaced_dots + ".txt");
                }
        ...
        string fileNamePART1 = graphFilename_replaced_dots + "-" + (removeLiterals ? "no_literals" : "with_literals");

        for (std::vector<weigher::GraphWeigher*>::const_iterator weigher = param.weighers.cbegin(); weigher != param.weighers.cend(); weigher++) {
            weigher::GraphWeigher * forwardWeigher = *weigher;
            c.reWeigh(*forwardWeigher);

            string fileNamePART2 = fileNamePART1 + "-forwardWeigher_" + forwardWeigher->getName();
            for (std::vector<double>::const_iterator alphas = param.alphas.cbegin(); alphas != param.alphas.cend(); alphas++) {
                ...
                for (std::vector<double>::const_iterator epss = param.epss.cbegin(); epss != param.epss.cend(); epss++) {
                    ...
                    for (std::vector<bool>::const_iterator onlyEntitiess = param.onlyEntities.cbegin(); onlyEntitiess != param.onlyEntities.cend(); onlyEntitiess++) {
                        ...
                        for (std::vector<bool>::const_iterator includeEdgess = param.includeEdges.cbegin(); includeEdgess != param.includeEdges.end(); includeEdgess++) {
                            bool includeEdges = *includeEdgess;

                            //creating filename
                            string outputFileName = "glove_input_file-" + fileNamePART2 + "-alpha_" + boost::lexical_cast<std::string>(alpha) + "-eps_" + boost::lexical_cast<std::string>(eps)
                                                    + (onlyEntities ? "-onlyEntities_yes" : "-onlyEntities_no") + (includeEdges ? "-edges_yes" : "-edges_no") + ".bin";

                            string outputPath = outputPrefix + "/" + outputFileName;

                            FILE* glove_input_file_out = fopen(outputPath.c_str(), "w");
